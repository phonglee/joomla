...
(function ($) {
    JSNPbLayoutCustomizer.prototype = {
        ...
        addRow: function (self, this_wrapper) {
           ...
           /* $("#jsn-add-read-more").click(function (e, get_chosen_layout) {
                e.preventDefault();
                var self = this;
                var html = '<div id="read-more" class="jsn-row-container ui-sortable row-fluid shortcode-container" style="">'+
                                '<div class="jsn-iconbar left">' +
                                    '<a href="javascript:void(0);" title="Move Up" class="jsn-move-up"><i class="icon-chevron-up"></i></a>'+
                                    '<a href="javascript:void(0);" title="Move Down" class="jsn-move-down"><i class=" icon-chevron-down"></i></a>'+
                                '</div>'+
                                '<textarea class="hidden" data-sc-info="shortcode_content" name="shortcode_content[]">[pb_row ]</textarea>'+
                                '<div class="jsn-pb-row-content">'+
                                    '<div class="jsn-column-container clearafter shortcode-container ">'+
                                        '<div class="jsn-column span12 ui-resizable" style="width: 1300px;">'+
                                            '<div class="thumbnail clearafter">'+
                                                '<textarea class="hidden" name="shortcode_content[]">[pb_column span="span12"]</textarea>'+
                                                    '<div class="jsn-column-content item-container" data-column-class="span12">'+
                                                        '<div class="jsn-handle-drag jsn-horizontal jsn-iconbar-trigger">'+
                                                            ...
                                                            '</div>'+
                                                        '</div>'+
                                                        '<div class="jsn-element-container item-container-content ui-sortable">'+
                                                            '<div class="jsn-item jsn-element ui-state-default jsn-iconbar-trigger shortcode-container EXTRA_CLASS  active-shortcode" data-el-type="element" data-name="Text" style="position: relative;">'+

                                                                '<textarea class="hidden  shortcode-content" shortcode-name="pb_text" data-sc-info="shortcode_content" name="shortcode_content[]">'+
                                                                    '[pb_text]&lt;hr id="system-readmore" /&gt;[/pb_text]'+
                                                                '</textarea>'+
                                                        ...
                                                        '</div>'+
                                                    '</div>'+
                                                    '<textarea class="hidden" name="shortcode_content[]">[/pb_column]</textarea>'+
                                                '</div>'+
                                            '<div class="ui-resizable-handle ui-resizable-e" style="z-index: 90; display: block;"></div>'+
                                        '</div>'+
                                    '</div>'+
                                '</div>'+
                                '<textarea class="hidden" name="shortcode_content[]">[/pb_row]</textarea>'+
                            '</div>';
                
                if (!$('#form-container #read-more').length) {
                    $('#form-container .jsn-row-container:last').after(html);
                    $('body').trigger('jsnpb_changed');
                }                    
                else{
